...
Sliding_Bloom::Sliding_Bloom(uint _STAGE_LEN, uint _HASH_NUM, uint _CYCLE):
    Basic(_STAGE_LEN * _HASH_NUM, _HASH_NUM), STAGE_LEN(_STAGE_LEN), STEP((double)_CYCLE / _STAGE_LEN * _HASH_NUM){
    Clock_Pos = 0;
    Last_Time = 0;
...
void Sliding_Bloom::Init(const Data &data, uint time){
    Clock_Go(time / STEP);
...
bool Sliding_Bloom::Query(const Data &data, uint time){
    Clock_Go(time / STEP);
    for(uint i = 0;i < HASH_NUM;++i){
        uint position = data.Hash(i) % STAGE_LEN + i * STAGE_LEN;
...
void Sliding_Bloom::Clock_Go(uint num){
