...
Forget_Bloom::Forget_Bloom(uint _LENGTH, uint _HASH_NUM, uint _BLOOM_NUM, uint _CYCLE):
    Basic(_LENGTH, _HASH_NUM), BLOOM_NUM(_BLOOM_NUM), STEP(_CYCLE / (_BLOOM_NUM - 1)){
    future = 0;
    Last_Time = 0;
...
void Forget_Bloom::Init(const Data &data, uint time){
    if(!Query(data, time)){
...
bool Forget_Bloom::Query(const Data &data, uint time){
    Update(time / STEP);
...
void Forget_Bloom::Update(uint time){
